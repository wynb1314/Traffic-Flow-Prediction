...
def api_topology_3d(request):
    ...
    edge_trace = go.Scatter(
        ...
        y=ey if ey else [0],
        mode='lines',
        line=dict(color='rgb(85, 84, 79)', width=1),
        hoverinfo='none',
        showlegend=False,
    )
    # 节点：颜色根据平均流量（拥堵程度）
    ...
    node_trace = go.Scatter(
        ...
        marker=dict(
            ...
            showscale=True,
            colorbar=dict(
                title=dict(text='平均流量', font=dict(color='#4a4a4a')),
                tickfont=dict(color='#7a7a7a'),
            ...
            line=dict(width=2, color='rgb(85, 84, 79)'),
        ),
    ...
    fig.update_layout(
        title=dict(text='2D 路网拓扑（307 节点 · 颜色=拥堵程度）', font=dict(size=16, color='#1a1a1a')),
